...
import { Calendar, Plus, Trash2, RefreshCw, Loader2, MapPin, ImageIcon, Pencil, Copy, AlertCircle, CheckCircle, Mountain } from 'lucide-react';
            ...
            <div className="bg-white rounded-2xl w-full max-w-5xl shadow-2xl flex flex-col max-h-[90vh]">
                <div className="p-6 border-b border-gray-100 flex justify-between items-center bg-[#002D72] text-white rounded-t-2xl">
                    <div>
                        ...
                        <p className="text-blue-200 text-sm">Manage stages, sync Strava, and validate results.</p>
                    </div>
                ...
                <div className="p-6 overflow-y-auto flex-1 bg-gray-50/50">
                    {isLoading ? (
                        <div className="flex flex-col items-center justify-center h-64">
                            ...
                            <p className="text-gray-500 font-medium">Loading stages...</p>
                        </div>
                    ) : (
                        <div className="space-y-6">
                                        ...
                                        <div className="p-6 flex-1 border-r border-gray-100">
                                            <div className="flex items-center justify-between mb-2">
                                                <div className="flex items-center gap-2">
                                                    <span className="bg-gray-100 text-gray-600 text-[10px] font-bold px-2 py-0.5 rounded uppercase tracking-wide">Stage {stage.stage_order}</span>
                                                    {needsReview && <span className="flex items-center gap-1 bg-orange-100 text-orange-700 text-[10px] font-bold px-2 py-0.5 rounded animate-pulse"><AlertCircle className="w-3 h-3" /> Review Required</span>}
                                                </div>
                                                <div className="flex gap-1">
                                                    ...
                                                    <button onClick={() => { if (confirm('Delete stage?')) deleteStage.mutate(stage.id) }} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded transition"><Trash2 className="w-4 h-4" /></button>
                                                </div>
                                                ...
                                                <div className="flex flex-wrap gap-4 text-sm text-gray-500 mt-3">
                                                    ...
                                                    <button
                                                        ...
                                                        className="flex items-center gap-1.5 text-orange-500 hover:text-orange-600 hover:bg-orange-50 px-2 py-0.5 rounded transition"
                                                    >
                                                        <Mountain className="w-4 h-4" /> Manage KOM Segments
                                                    </button>
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-6 bg-gray-50 w-full md:w-72 flex flex-col justify-between gap-4">
                                                <div className="grid grid-cols-2 gap-3">
                                                    ...
                                                    <div className="p-3 bg-white rounded-lg border border-green-100 shadow-sm flex flex-col items-center">
                                                        ...
                                                        <span className="text-[10px] font-bold text-gray-500 uppercase">Official</span>
                                                    </div>
                                                </div>
                            ...
                            {!isCreating && (
                                ...
                                >
                                    <Plus className="w-6 h-6" /> Add New Stage
                                </button>
                            )}

                            {isCreating && (
                                <div className="bg-white p-6 rounded-2xl border-2 border-[#009CDE] shadow-xl animate-fade-in">
                                    <h4 className="font-bold text-gray-800 mb-6 text-lg">{editingStageId ? 'Edit Stage' : 'New Stage'}</h4>
                                    <form onSubmit={handleCreate} className="space-y-4">
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            ...
                                            <div>
                                                <label className="text-xs font-bold text-gray-500 uppercase">Segments (IDs)</label>
                                        ...
                                        <div className="flex justify-end gap-3 pt-4">
                                            <button type="button" onClick={() => { setIsCreating(false); setEditingStageId(null); }} className="px-6 py-2 text-gray-500 font-bold hover:bg-gray-100 rounded-lg">Cancel</button>
                                            <button type="submit" className="px-8 py-2 bg-[#009CDE] text-white font-bold rounded-lg hover:bg-blue-600 shadow-lg">{editingStageId ? 'Save' : 'Create'}</button>
                                        </div>
                                    </form>
                                </div>
                            )}
